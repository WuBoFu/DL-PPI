...
def test(model, graph, test_mask, device):
    ...
    print("Accuracy: {}, Recall: {}, Precision: {}, F1: {}".format(metrics.Accuracy, metrics.Recall, metrics.Precision, metrics.F1))
...
def main():
    ...
    with open(args.index_path, 'r') as h:
    ...
    model_dict = model.state_dict()  #现模型参数
 
    state_dict = torch.load(args.gnn_model,map_location='cpu')  #加载旧模型
 
        #我这里是下面三层不一样，需要过滤掉
    pretrained_dict = {k: v for k, v in state_dict.items() if k in model_dict and k not in ["conv1d.weight"]}
    for k ,v in pretrained_dict.items(): #通过k,v来查看是那几层模型结构不一样
        print(k,v.size())
    model_dict.update(pretrained_dict) #更新现模型没有变的部分参数为旧模型参数
    
    for k, v in model_dict.items(): #
        print(k,v.size())
    model.load_state_dict(model_dict) #加载新的模型参数



    # model.load_state_dict(torch.load(args.gnn_model,map_location='cpu')['state_dict'])
